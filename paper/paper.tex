...
We now provide a sample implementation of a variational store based on the variational map data structure \cite{varwhile, walkingshaw2014variational}.
...
The variational values in the map must be of type \prog{Maybe v} in order to account for cases where a value is assigned in one variant but not in others.
...
vmerge :: VOpt v -> Cond -> VOpt v -> -> VOpt v
vmerge (One v) vctx v' = Chc vctx v' (One v)
vmerge v@(Chc c l r) vctx v'@(One _)
  | c => vctx = Chc vctx (One v) (vmerge )
  | vctx => c = Chc c (vmerge l vctx (One v)) r
  | vctx => Not c = Chc c l (vmerge r vctx (One v))
  | otherwise = Chc c (vmerge l vctx (One v)) (vmerge r vctx (One v))
vmerge 
...
vupdate c k v vst = 
  where
    mkOpt = fmap Just
    current = lookup k vst
    case current of
      Nothing -> insert k (Chc c (mkOpt v) (One Nothing)) vst
      Just v -> insert k (vmerge c v current) vst
\end{lstlisting}
...
\def \BigVCatch {\infer [VS-Catch] {(C,S,[]:E,s) \Downarrow_{V} (S',l:E) \\ (\GetCtx{C}{(l:[])},S'[(\GetCtx{C}{E},x) \rightarrow \mathit{combine}(l)],E,s') \Downarrow_{V} (S'',E')} {(C,S,E,\CCkeyw{try}\ s\ \CCkeyw{catch}\ x\ \CCkeyw{with}\ s') \Downarrow_{VS} (S'',E')}}
