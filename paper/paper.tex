...
challenges in the interaction of variation and side effects. Previous variational
...
strategies for systematically simplifying variational values \cite{ericthesis}. The use of formula
trees permits additional simplification operations \cite{hubbard2016formula,walkingshaw2014projectional}.
...
such cases are easily detected and prevented with a proper type system. Variational type systems have been studied extensively and readers seeking static guarantees
that undefined variable access is impossible are referred to those works \cite{kenner2010typechef,chen2012error,chen2014extending}.
...
Short of integrating a BDD solver, there are other optimization or simplification operations that could be performed \cite{ericthesis,hubbard2016formula}, but which to perform and how frequently
...
the parent choice should set its flag, indicating that execution is halted for all possible variants. In the Resource DSL, we implement this system via an error monad,
...
The judgments for variational expressions require a triple of the current variational context, the variable store, and the expression, producing a variational integer or boolean.
...
type of the parameter may depend on the variant it is located in, we define types to be a choice of primitive types. The typechecking of the Resource DSL is further discussed
in Section \ref{sec:typecheck}.
...
After all, we have an existing monad instance for formula trees. This would provide the benefit of easily integrating choices in the highly
...
type MonadEval m = (MonadError (V (Maybe Error)) m, MonadReader RCtx m, MonadState StateCtx m)
...
allows us to return a default value from alternatives that are in an error state with minimal
...
type, we define it as a standalone function. Of note is that vbind demonstrates the sort of flag-based exception handling optimization
...
vHandle :: MonadEval m =>
    ...
    V (Maybe a) ->
    m (V (Maybe b))
vHandle c f l r = vHandle' c (f l) (f r)
...
vBind v f = do
  ...
  case v' of
    ...
    (One (Just a)) -> f a
    (Chc c l r) -> vHandle c ($\lambda$v -> vBind (return v) f) l r
\end{lstlisting}
...
Throwing exceptions is handled by the function \prog{verror}, which updates the error context and the error value before
throwing an exception for the current variant:
...
variational lookup operation for the resource environment, omitting the implementations for modification, deletion, and creation.
...
\begin{lstlisting}
vLookup' :: (MonadEval m, Ord k) => k -> Map k (V (Maybe v)) -> m (V (Maybe v))
vLookup' k env
...
vLookup :: (MonadEval m, Ord k) => Error -> k -> Map k (V (Maybe v)) -> m (V (Maybe v))
vLookup e k env = do
  v <- vLookup' k env
  checkExists e v
  return v
...
are interested in and the query returns whether that condition has any successful configurations.
...
dimension, along with the various provider libraries. This allowed for our variational interpreter to potentially check all possible
configurations at once. This also meant that if a user were only interested in a particular subset of features, such as only AES
